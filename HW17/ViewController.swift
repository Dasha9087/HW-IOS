...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setupUI()
        configureButtons()
        buttonsStackView.onButtonTap = { [weak self] item in
            self?.handleButtonTap(item)
        }
    }
    ...
    private func handleButtonTap(_ item: CalculatorButtonItem) {
        if let title = item.title {
            ...
            case "%":
                ...
                resultLabel.text = formatNumber(currentNumber)
            case "+/-":
                currentNumber *= -1
                resultLabel.text = formatNumber(currentNumber)
            case ",":
...
final class ButtonStackView: UIView {
    var onButtonTap: ((CalculatorButtonItem) -> Void)?
